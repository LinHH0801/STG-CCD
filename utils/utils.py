...
def FaM(pred, label, num_classes, ignore_zero=False):

    # 展平
    pred = pred.ravel()
    label = label.ravel()

    # 构建掩码：label >= 0；若 ignore_zero，则 label > 0
    valid = (label >= 0)
    if ignore_zero:
        valid = valid & (label > 0)
    ...
    f1_mean = np.mean(f1_per_class[valid_classes])
...
def FaM(pred, label, num_classes, ignore_zero=False):
    ...
    return miou,f1_mean
def SCDD_eval_binary(preds, labels, num_class):
    hist = np.zeros((num_class, num_class))
    ...
    #Accuracy
    Accuracy = np.diag(c2hist).sum() / c2hist.sum()
    #Precision
    Precsion = hist_fg.sum()/c2hist.sum(1)[1]
    Recall = hist_fg.sum()/c2hist.sum(0)[1]
    F1 = (2*Recall*Precsion)/(Precsion+Recall)
    iu = np.diag(c2hist) / (c2hist.sum(1) + c2hist.sum(0) - np.diag(c2hist))
    IoU_fg = iu[1]
    IoU_mean = (iu[0] + iu[1]) / 2
    return Accuracy, Precsion,Recall,F1,IoU_mean
