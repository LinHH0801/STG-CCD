...
def rand_flip_MCD(img1, img2, img3, label12,label23,label):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img1, img2, img3,  label12,label23,label
    elif r < 0.5:
        return (np.flip(img1, axis=0).copy(), np.flip(img2, axis=0).copy(), np.flip(img3, axis=0).copy(),
                np.flip(label12, axis=0).copy(), np.flip(label23, axis=0).copy(), np.flip(label, axis=0).copy())
    elif r < 0.75:
        return (np.flip(img1, axis=1).copy(), np.flip(img2, axis=1).copy(), np.flip(img3, axis=1).copy(),
                np.flip(label12, axis=1).copy(), np.flip(label23, axis=1).copy(),  np.flip(label, axis=1).copy())
    else:
        return (
        img1[::-1, ::-1, :].copy(), img2[::-1, ::-1, :].copy(), img3[::-1, ::-1, :].copy(),
        label12[::-1, ::-1].copy(),label23[::-1, ::-1].copy(),label[::-1, ::-1].copy())


def rand_rot90_MCD(img1, img2, img3,label12,label23,label):
    r = random.random()
    if r < 0.5:
        return img1, img2, img3,  label12,label23,label
    else:
        return (np.rot90(img1).copy(), np.rot90(img2).copy(), np.rot90(img3).copy(),
                np.rot90(label12).copy(),np.rot90(label23).copy(),np.rot90(label).copy())


def rand_rot90_flip_MCD(img1, img2, img3, label12,label23,label):
    img1, img2, img3,  label12,label23,label = rand_rot90_MCD(img1, img2, img3, label12,label23,label)
    return rand_flip_MCD(img1, img2, img3, label12,label23,label)
...
def data_padding(imgs, labels, scale=32):
    for idx, img in enumerate(imgs):
        ...
        if need_padding:
            ...
            print(
                'img padding: [%d, %d]->[%d, %d]' % (shape_before[0], shape_before[1], shape_after[0], shape_after[1]))
            imgs[idx] = img
            labels[idx] = label
    return imgs, labels
...
def random_crop_MCD(img1, img2, label1, label2, size):
    # print(img.shape)
    h = img1.shape[0]
    w = img1.shape[1]
    c_h = size[0]
